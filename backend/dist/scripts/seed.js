...
async function seed() {
    ...
    try {
        ...
        for (const userData of demoUsers) {
            ...
            const empRes = await client.query('INSERT INTO employees (name, email, department, designation, "joiningDate", status) VALUES ($1, $2, $3, $4, NOW(), $5) RETURNING id, email, name', [userData.name, userData.email, userData.department, userData.designation, 'active']);
            const newEmployee = empRes.rows[0];
            createdEmployees.push(newEmployee);
            await client.query('INSERT INTO users ("employeeId", email, password, role) VALUES ($1, $2, $3, $4)', [newEmployee.id, newEmployee.email, hashedPassword, userData.role]);
        ...
        const skillRes = await client.query(`INSERT INTO skills (name, category) SELECT unnest($1::text[]), unnest($2::text[]) RETURNING id`, [skillsToCreate.map(s => s.name), skillsToCreate.map(s => s.category)]);
        ...
        await client.query(`INSERT INTO goals ("employeeId", title, description, type, status, progress, weightage) SELECT "employeeId", title, description, type, status, progress, weightage FROM jsonb_to_recordset($1) AS x("employeeId" uuid, title text, description text, type text, status text, progress int, weightage int)`, [JSON.stringify(goalsToCreate)]);
        ...
        await client.query(`INSERT INTO reviews (cycle_id, reviewee_id, reviewer_id, rating, feedback, type, status) SELECT cycle_id, reviewee_id, reviewer_id, rating, feedback, type, status FROM jsonb_to_recordset($1) AS x(cycle_id uuid, reviewee_id uuid, reviewer_id uuid, rating int, feedback text, type text, status text)`, [JSON.stringify(reviewsToCreate)]);
        ...
        await client.query(`INSERT INTO skill_assessments ("employeeId", "skillId", "selfRating", "managerRating") SELECT "employeeId", "skillId", "selfRating", "managerRating" FROM jsonb_to_recordset($1) AS x("employeeId" uuid, "skillId" uuid, "selfRating" int, "managerRating" int)`, [JSON.stringify(assessmentsToCreate)]);
